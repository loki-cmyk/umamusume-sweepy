...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update skills JSON database from GameTora Skill List HTML.")
    parser.add_argument(
        "--html_path",
        required=True,
        help="Path to the source GameTora Skill List HTML file."
    )
    parser.add_argument(
        "--extracted_path",
        default=os.path.join(project_root, "scratch", "extracted_skills.json"),
        help="Path where the parsed skill dictionary will be temporarily saved."
    )
    parser.add_argument(
        "--db_path",
        default=os.path.join(project_root, "web", "src", "assets", "umamusume_final_skills_fixed.json"),
        help="Path to the final skills JSON database file."
    )
    ...
    if not os.path.exists(args.db_path):
        print(f"Error: JSON database file not found at {args.db_path}")
        return
        
    extracted_skills = parse_html_skills(args.html_path)
    
    print(f"Saving extracted skills list to {args.extracted_path}...")
    with open(args.extracted_path, 'w', encoding='utf-8') as f:
        json.dump(extracted_skills, f, indent=2, ensure_ascii=False)
        
    print("Updating the JSON database...")
    update_count = update_skills_json(args.db_path, extracted_skills)
